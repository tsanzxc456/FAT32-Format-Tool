================================================================================
    MFC 程式庫 : FAT32_Format_Tool 專案概觀
===============================================================================

應用程式精靈已經為您建立了這個 FAT32_Format_Tool 應用程式。這個應用程式
不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。

這個檔案的內容摘要，包含各個構成 FAT32_Format_Tool 應用程式的檔案。

FAT32_Format_Tool.vcxproj
    這是使用應用程式精靈所產生之 VC++ 專案的主要專案檔。 
    它包含產生檔案時的 Visual C++ 版本資訊，以及使用應用程式精靈產生檔案的
    過程中，選取的平台、組態和專案功能的相關資訊。

FAT32_Format_Tool.vcxproj.filters
...
FAT32_Format_Tool.h
    ...
    並宣告 CFAT32_Format_ToolApp 應用程式類別。

FAT32_Format_Tool.cpp
    這是主要的應用程式原始程式檔，它包含應用程式類別 CFAT32_Format_ToolApp。

FAT32_Format_Tool.rc
    這份清單包含了所有程式要使用的 Microsoft Windows 資源。其中包括了儲存
    在 RES 子目錄下的圖示、點陣圖和游標。您可以直接在 Microsoft Visual C++ 
    內編輯這個檔案。您的專案資源在 1028。

res\FAT32_Format_Tool.ico
    這個圖示檔是用來做為應用程式的圖示。這個圖示包含在主要資源檔
    FAT32_Format_Tool.rc 內。

res\FAT32_Format_Tool.rc2
...
FAT32_Format_ToolDlg.h、FAT32_Format_ToolDlg.cpp - 對話方塊
    這些檔案包含您的 CFAT32_Format_ToolDlg 類別。這個類別定義應用程式主
    對話方塊的行為。對話方塊的範本在 FAT32_Format_Tool.rc 內，您可以 
...
StdAfx.h、StdAfx.cpp
    這些檔案是用來建置名為 FAT32_Format_Tool.pch 的先行編譯標頭 (PCH) 檔，
